...
import type { Interview, InterviewFormJSON, InterviewProgress, InterviewResult, Step } from '@/types/interview';
...
export const useUserStore = defineStore('user', () => {
    ...
    const currentInterview = computed(() => {
        let result: Interview | null | undefined = interviews.value.find(
            (item) => item.id === currentInterviewNode.value
        );
    ...
    const currentInterviewProgress = computed(() => {
        ...
        const result = interviewProgress.value.filter((item) => item.campaign.id === currentInterviewNode.value);
    ...
    const steps = computed(() => {
        ...
        if (currentInterviewProgress.value) {
            currentInterviewProgress.value.forEach((item) => {
                Steps.push({
                    ...
                    state: (function () {
                        const session = item.campaign.stage.session;
                        if (isUnderWay.value && isBeforeOrBetweenNow(session.start_time, session.end_time))
                            return 'active';
        ...
        Steps = [
            ...Steps,
            {
                ...
                state: (function () {
                    if (isUnderWay.value && (!waitingInterview.value || !currentInterviewProgress.value))
                        return 'active';
            ...
            },
            {
                step: Steps.length + 2,
                title: '面试结束',
                description: '面试结束后，您可以在此网站中查看面试结果，也可以通过邮件或其他方式获取通知。',
    ...
    function initInterviewData() {
        return Promise.allSettled([getInterviewList(), getUserInterviewProgress(), getInterviewResult()]);
    }
