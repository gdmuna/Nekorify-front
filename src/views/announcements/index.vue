<template>
    <div class="pt-20 px-1 md:px-2 lg:px-3 xl:px-4 pb-10 flex-1 flex flex-col">
        ...
        <template v-else>
            <h2 class="md:text-3xl text-2xl text-center">{{ currentAnnouncement?.title }}</h2>
...
const sectionData = computed(() => {
    ...
    return currentAnnouncement.value
        ? [{ title: currentAnnouncement.value.title, resourceUrl: currentAnnouncement.value.text_md_url }]
        : [];
});
