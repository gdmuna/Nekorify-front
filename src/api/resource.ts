...
import type { UploadSourceRes, UploadAnnouncementRes, UploadArticleRes } from '@/types/resource';
...
export const resourceApi = {
    ...
    async uploadAnnouncement(data: { title: string; department: string; coverUrl?: string; textUrl: string }) {
        const inst = nekorify.Post('/announcement', data);
        return await to<UploadAnnouncementRes>(inst);
    },
    async uploadArticle(data: { title: string; coverUrl?: string; textUrl: string }) {
