    ...
    <div class="flex flex-col space-y-4 xl:text-xl md:text-[1rem] xl:w-[19.5rem] md:w-[16rem]">
        <div class="flex flex-row md:flex-col md:space-y-4 md:space-x-0 space-x-4 md:items-start items-center">
            <img :src="userInfo.avatar" alt=""
                class="xl:size-78 md:size-64 size-24 rounded-full object-cover border-2 dark:border-[#E0DEC0] select-none">
            <div class="text-3xl">
        ...
        <secondaryButton text="登出" :icon="LogOut" class="dark:bg-[#f19180] dark:text-[#0E100F] rounded xl:text-xl md:text-[1rem] md:mt-4 mt-2 w-fit" @click="logout" />
    </div>
</template>
...
import { PenLine, UsersRound, Mail, Link, Hash, BadgeInfo, CircleDollarSign, LogOut } from 'lucide-vue-next';
