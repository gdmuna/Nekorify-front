    ...
    <div class="mx-10 mt-5 mb-10 flex justify-center">
        <div v-if="showItem" class="flex flex-col">
            <div class="flex md:flex-row flex-col md:space-x-4 space-y-4">
                <div
                    class="xl:w-180 md:w-120 w-78 md:h-80 h-40 rounded-xl dark:bg-[#CAB8A4] flex items-center justify-center shrink-1">
                    video{{ currentIdx + 1 }}
                </div>
                <div class="flex space-x-4 md:-translate-x-0 -translate-x-6">
                    <div
                        ref="indicator"
                        class="md:size-4 size-3 dark:bg-amber-100 rounded-full will-change-transform" />
                    <div ref="titles" class="flex flex-col space-y-4">
                        <div
                            ...
                            :data-index="index">
                            <p
                                class="xl:text-4xl md:text-3xl text-2xl title cursor-pointer"
                                @click="toggle('custom', index)">
                                {{ item.title }}
                            </p>
        ...
        <h2 v-else class="text-center text-2xl md:text-3xl dark:text-[#A0A0A0] my-10">还没有视频喵...</h2>
    </div>
</template>
...
.subtitle-slide-leave-active {
    transition: all 0.5s cubic-bezier(0.6, 1.8, 0.33, 0.96);
}
