        ...
        <template #main>
            <div class="flex-1 flex-col items-start justify-start w-full h-full space-y-4 m-2 text-sm md:text-[1rem]">
                ...
                <p ref="pTag3" class="critical-shadow text-[#FF5F56] font-bold md:text-lg mt-16">
                    ...
                    <span class="text">FULL-STACK NODES REQUIRED</span>
                </p>
                <p ref="pTag4" class="text-[#FFBD2E] font-bold mt-8 mb-4">> ACTION REQUIRED:</p>
                <button
                    ref="Button"
                    class="px-4 py-2 text-[#0E100F] rounded-md font-bold cursor-pointer mb-2 will-change-transform relative"
...
onMounted(() => {
    gsap.set([pTag2.value!.querySelector('.log-level'), pTag3.value!.querySelector('.log-level'), pTag4.value], {
        autoAlpha: 0
    });
