        ...
        <template v-if="!showDetail">
            <section class="space-y-2">
                ...
                <template v-if="articleFetch.dataStatus.value === 'loaded'">
                    ...
                    <div v-if="activateArticle.data.value && activateArticle.data.value.length > 0">
                        <liItem v-for="(item, index) in activateArticle.data.value" :key="index" useSlot
...
const activateArticle = {
    data: computed(() => {
        if (articleFetch.data.value) {
            const val = articleFetch.data.value
