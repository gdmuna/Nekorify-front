<template>
    <div class="flex-1 flex flex-col">
        <div v-if="!showDetails" class="space-y-4">
            ...
            <section>
                <liItem useSlot v-for="(item, index) in interviews" :key="item.id" :rightIcon="ArrowRight"
...
const { getInterviewList, getUserInterviewProgress, getInterviewResult } = userStore;
const { interviews } = storeToRefs(userStore)
...
onMounted(async () => {
    const tasks = []
    let task = getInterviewList()
    tasks.push(task)
    task = getUserInterviewProgress()
    tasks.push(task)
    task = getInterviewResult()
    tasks.push(task)
