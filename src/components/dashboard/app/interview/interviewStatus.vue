                ...
                <div ref="stepRoot" v-if="steps !== null" id="steps" class="mt-6 md:ml-4 md:gap-4">
                    <Stepper orientation="vertical" :defaultValue="activeStepIndex + 1" :linear="false"
                        ...
                        <StepperItem v-for="(step, index) in steps" :key="index" v-slot="{ state }"
                            class="relative flex items-start gap-6 flex-1" :step="step.step">
                            ...
                            <teleport defer v-if="state === 'active'" :to="teleportTo(index, step.type)">
                                ...
                                <div v-else-if="step.type === 'process'"
                                ...
                                <div v-else-if="step.type === 'result'" class="bg-[#f7f3ed] dark:bg-[#181818] rounded-xl md:p-8 p-6 max-w-2xl
                                col-span-5 md:col-span-3 mx-auto shadow-lg space-y-6 my-auto md:text-lg">
                                    <template v-if="step.result === 'approved'">
                                        <div class="title font-poem text-3xl md:text-3xl font-bold flex flex-col w-fit mx-auto">
                                        ...
                                        <h3 class="text-2xl md:text-3xl font-bold text-emerald-500 mb-2">🎉 恭喜通过面试！</h3>
                                        <p class="text-lg md:text-2xl text-amber-100">
                                            {{ userInfo.username }}同学，见信欢愉，谨以为贺！
                                        </p>
                                        <p>
                                            恭喜你成功通过
                                            <span v-if="currentAssociation" v-for="(item, index) in currentAssociation"
                                                :key="index" class="font-bold text-emerald-500">{{ item.label }}
                                                <span v-if="index < currentAssociation.length - 1">& </span>
                                            </span>
                                            的干事招新面试，成为
                                            <span v-if="currentDepartment" v-for="(item, index) in currentDepartment"
                                                :key="index" class="font-bold text-emerald-500">{{ item.label }}
                                        ...
                                        <p>
                                            ...
                                            <span class="font-bold text-emerald-500">愿你不坠青衿之志，常怀臻于至善之心，脚踏实地，也仰望星空。</span>
                                        </p>
                                        <p>
                                            谨以此言相赠：
                                        </p>
                                    ...
                                    <template v-if="step.result === 'rejected'">
                                        ...
                                        <p>
                                            ...
                                            <span class="font-bold text-emerald-500">他日若你仍怀此志，我们愿再度为你敞开大门。</span>
                                        </p>
                                        <p>
                                            前路广阔，天地浩荡。愿你：
                                        </p>
