...
// 校验规则
const normalFormSchema = toTypedSchema(
    z.object({
        nickname: z
            .string()
...
const passwordFormSchema = toTypedSchema(
    z
        .object({
            oldPassword: z
                .string({ invalid_type_error: `原密码 必须是字符串`, required_error: `原密码 不能为空` })
                .min(6, { message: '原密码 长度需大于等于 8' })
                .max(50, { message: `原密码 长度需小于等于 50` })
                .nonempty({ message: `原密码 不能为空` }),
            newPassword: z
                .string({ invalid_type_error: `新密码 必须是字符串`, required_error: `新密码 不能为空` })
                .min(6, { message: '新密码 长度需大于等于 8' })
                .max(50, { message: `新密码 长度需小于等于 50` })
                .nonempty({ message: `新密码 不能为空` }),
            confirmPassword: z
                .string({ invalid_type_error: `确认密码 必须是字符串`, required_error: `确认密码 不能为空` })
...
const emailFormSchema = toTypedSchema(
    z.object({
        email: z
            .string({ invalid_type_error: `新邮箱 必须是字符串`, required_error: `新邮箱 不能为空` })
            .email({ message: '新邮箱 格式不正确' })
            .nonempty({ message: `新邮箱 不能为空` }),
        emailVerificationCode: z.preprocess(
            (val) => {
            ...
            z
                .number({ invalid_type_error: `邮箱验证码 必须是数字`, required_error: `邮箱验证码 不能为空` })
